...
class Config:
    def __init__(self):
        parser = argparse.ArgumentParser(description="Hybrid Poker RL Training")
        parser.add_argument('--base_dir', type=str, default='./poker_hybrid', help='Base directory for models and logs')
        parser.add_argument('--num_workers', type=int, default=8, help='Number of parallel workers')
        parser.add_argument('--num_episodes', type=int, default=10, help='Number of training episodes')
        args = parser.parse_args()

        self.BASE_DIR = args.base_dir
        self.MODEL_PATH = os.path.join(self.BASE_DIR, 'models', 'hybrid_model.pt')
        self.BEST_MODEL_PATH = os.path.join(self.BASE_DIR, 'models', 'hybrid_best.pt')
        ...
        self.REWARD_NORMALIZATION = 'stack'  # или 'stats' для варианта из #work2
...
class RewardNormalizer:
    def __init__(self, eps: float = 1e-8, max_size: int = 10000, mode: str = 'stack'):
        self.rewards = []
        self.eps = eps
        ...
        self.mean = 0.0
        self.std = 1.0
        logging.info(f"RewardNormalizer инициализирован с mode={mode}")

    def update(self, reward: float, stack: float = 1.0):
        try:
            if self.mode == 'stack':
                reward = reward / max(stack, 1e-8)
            ...
            logging.error(f"Ошибка в RewardNormalizer.update: {traceback.format_exc()}")
            raise

    def normalize(self, reward: float, stack: float = 1.0) -> float:
        if not self.rewards:
            return reward if self.mode != 'stack' else reward / max(stack, 1e-8)
        if self.mode == 'stack':
            reward = reward / max(stack, 1e-8)
...
class OpponentStats:
    def __init__(self):
        self.stats = {i: {'vpip': 0, 'pfr': 0, 'af': 0, 'hands': 0, 'folds': 0, 'calls': 0, 'raises': 0} 
...
class StateProcessor:
    ...
    def _load_or_precompute_buckets(self):
        ...
        kmeans = KMeans(n_clusters=config.NUM_BUCKETS, random_state=42).fit(np.array(all_hands))
...
class PokerAgent(policy.Policy):
    ...
    def _hand_strength(self, state, player_id: int) -> float:
        ...
        cards = [int(c) for i, c in enumerate(info[:52]) if c > 0][:2]
        if len(cards) < 2:
            return 0.1
    ...
    def _heuristic_reward(self, action: int, state, player_id: int, bets: List[float], opponent_stats: OpponentStats) -> float:
        ...
        elif action > 1 and any(m['pfr'] > 0.5 for m in opp_metrics):
    ...
    def action_probabilities(self, state, player_id: Optional[int] = None, opponent_stats: Optional[OpponentStats] = None) -> Dict[int, float]:
        ...
        probs = positive_regrets / regret_sum if regret_sum > 0 else np.ones(len(legal_actions)) / len(legal_actions)
        return {legal_actions[i]: float(probs[i]) for i in range(len(legal_actions))}
    ...
    def step(self, state, player_id: int, bets: List[float], stacks: List[float], stage: List[int], opponent_stats: OpponentStats) -> int:
        ...
        action = random.choices(list(probs.keys()), weights=list(probs.values()), k=1)[0] if random.random() < epsilon else max(probs, key=probs.get)
        if action > 1:
            state_tensor = torch.tensor(self.processor.process([state], [player_id], [bets], [stacks], [stage]), 
                                        dtype=torch.float32, device=device)
            self.strategy_net.eval()
    ...
    def update_strategy_pool(self, winrate: float = None):
        strategy_dict = {}
        ...
        if winrate is not None and winrate > self.best_winrate * 0.95:
            self.strategy_pool.append({'weights': self.strategy_net.state_dict(), 'winrate': winrate})
            self.best_winrate = max(self.best_winrate, winrate)
        elif len(self.strategy_pool) < config.MAX_STRATEGY_POOL:
            self.strategy_pool.append({'weights': self.strategy_net.state_dict(), 'winrate': 0.0})
        if len(self.strategy_pool) > config.MAX_STRATEGY_POOL:
            self.strategy_pool = sorted(self.strategy_pool, key=lambda x: x['winrate'], reverse=True)[:config.MAX_STRATEGY_POOL]
        self.cumulative_strategies.clear()
        logging.info(f"Strategy pool updated, size: {len(self.strategy_pool)}, best_winrate: {self.best_winrate}")

# ===== СБОР ДАННЫХ =====
@ray.remote(num_cpus=1, num_gpus=0.125)
def collect_experience(game, agent: PokerAgent, processor: StateProcessor, steps: int, worker_id: int):
    try:
...
def run_tournament(game, agent: PokerAgent, processor: StateProcessor, num_games: int = 50) -> float:
    total_reward = 0
    ...
    for _ in range(num_games):
        ...
        while not state.is_terminal():
            ...
            if len(board_cards) == 0:
                ...
                stage[3] = 1
            opponent_stats = OpponentStats()
            action = agent.step(state, player_id, bets, stacks, stage, opponent_stats) if player_id == 0 else tag_agent.action_probabilities(state, player_id).get(max(tag_agent.action_probabilities(state, player_id), key=tag_agent.action_probabilities(state, player_id).get))
            state.apply_action(action)
        total_reward += state.returns()[0]
    avg_reward = total_reward / num_games
    logging.info(f"Tournament avg reward: {avg_reward:.4f}")
    return avg_reward
...
class Trainer:
    def __init__(self, game, agent: PokerAgent, processor: StateProcessor):
        ...
        self.reward_normalizer = RewardNormalizer(mode=config.REWARD_NORMALIZATION)
        self.global_step = 0
        self.beta = 0.4
    ...
    def load_checkpoint(self):
        ...
        self.reward_normalizer.rewards = checkpoint.get('reward_buffer', [])
    ...
    def save_checkpoint(self, is_best=False):
        ...
        if is_best:
            ...
            logging.info(f"Saved best model to {config.BEST_MODEL_PATH}")
        self.last_checkpoint_time = time.time()
        logging.info(f"Saved checkpoint to {config.MODEL_PATH}")
        ...
        for episode in range(config.NUM_EPISODES):
            ...
            if experiences:
                ...
                for exp in experiences:
                    state, player_id, action, reward, next_state, done, bets, stacks, stage = exp
                    self.reward_normalizer.update(reward, stacks[player_id])
