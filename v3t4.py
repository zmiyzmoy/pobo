...
class PokerAgent(policy.Policy):
    ...
    def step(self, state, player_id: int, bets: List[float], stacks: List[float], stage: List[int], opponent_stats: OpponentStats) -> int:
        legal_actions = state.legal_actions(player_id)  # Получаем список легальных действий
        ...
        if total_weight <= 0:
            ...
            if not all(a in legal_actions for a in actions):
                logging.error(f"Non-legal actions in probs: {probs}, legal: {legal_actions}")
                action = random.choice(legal_actions)  # Fallback
            else:
                ...
                if action > 1:
                    ...
                    if bet_size >= stacks[player_id] and 3 in legal_actions:
                        action = 3
                    elif action not in legal_actions:  # Дополнительная проверка после изменения
                        action = random.choice(legal_actions)
        logging.debug(f"Player {player_id} chose action {action} from legal actions {legal_actions}")
        return action
...
def collect_experience(game, agent, processor, steps, worker_id):
    ...
    for _ in range(steps):
        ...
        legal_actions = env.legal_actions(player_id)  # Получаем легальные действия
        if player_id < 0:
            action = random.choice(legal_actions)
        ...
        if action not in legal_actions:
            logging.error(f"Worker {worker_id}: Invalid action {action}, legal: {legal_actions}")
            action = random.choice(legal_actions)  # Fallback
